...
if uploaded_file is not None:
    ...
    top_src=(
        ...
        else pd.DataFrame(columns=["Source IP","Packets"])
    )
    with c1:
        st.write("Top Source & Destination IPs")
    ...
    top_dport=(
        pd.DataFrame( 
            stats["Dst_Port_counter"].most_common(10),
            columns=["Destination Port","Packets"],
        )
        if stats["Dst_Port_counter"]    
        else pd.DataFrame(columns=["Destination Port","Packets"])
    )
    st.dataframe(top_dport,use_container_width=True)
